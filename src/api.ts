  ...
  "topImprovements": [
    {
      ...
      "action": "<具体的な改善アクション。デザイナーや実装者がそのまま作業できるレベルで、位置・サイズ・色・文言などを指定して記述>",
      "steps": [
        "<実装ステップ1: 最初にやること>",
        "<実装ステップ2: 次にやること>",
        "<実装ステップ3: 仕上げ>"
      ...
      "claudeCodePrompt": "<この改善をClaude Codeで実装するための具体的な指示文。「〜のファイルの〜セクションを開き、ヘッドラインを『〜』に変更して」のように、そのままClaude Codeにコピペして使える実践的なプロンプトを書く。HTMLやコンポーネントの修正箇所、変更内容、追加すべき要素を明確に指示する>"
    }
  ],
  "strategicAdvice": [
    {
...
- actionはデザイナーや実装者への指示書のように、位置・サイズ・色・文言を具体的に指定してください
- stepsは3〜5ステップで、実装の優先順をつけて書いてください
- expectedImpactには、ルーブリック内の業界ベンチマーク数値を引用し、改善前後の差を定量的に示してください
- reasonは3文以上で、行動心理学やCROの原則を交えて論理的に説明してください
- claudeCodePromptは、Claude Code（AIコーディングアシスタント）にそのままコピペして使える指示文を書いてください。「index.htmlの〜セクションで」「hero-sectionコンポーネントの」のように、一般的なLP実装を想定したファイル・セクション指定を含めてください。テキスト変更だけでなく、要素の追加・レイアウト変更・スタイル調整なども含めてください
